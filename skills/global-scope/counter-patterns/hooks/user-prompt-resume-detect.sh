...
if echo "$PROMPT" | grep -qiE 'resum(e|ing)|continue (from|the)|pick up|where (we|did) (left|leave)|carry on|where were we'; then
  cat <<'EOF'
## Resume detected (CP-010)

The user's message looks like a resume / continuation. Counter-pattern policy:
...
if echo "$PROMPT" | grep -qiE '^(actually|wait|stop|hold on|let.?s shift|pivot|instead|change of plans|new direction)'; then
  cat <<'EOF'
## Pivot detected (CP-011)

The user's message looks like a mid-session pivot. Counter-pattern policy: explicitly summarize what you're abandoning and what you're keeping before continuing. Do not silently mix old and new direction.
EOF
  exit 0
fi
