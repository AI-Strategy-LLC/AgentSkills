#!/usr/bin/env bash
# onedrive-walk-extract.sh — Download OneDrive candidate files and extract
# text excerpts for the ato-doc-summarizer agent.
#
# OneDrive for Business is a per-user personal SharePoint site, so this
# script uses the same `m365 spo file get --asFile` command as the
# SharePoint walker. It differs only in: (a) `--source onedrive` defaults,
# (b) the candidates JSON carries `user_upn` per entry, which gets routed
# through `source_meta.user_upn` in the manifest so the calling skill's
# Step 5 can prefix evidence files with the user's UPN-local-part on
# cross-user filename collisions.
#
# Strictly read-only on the tenant. Never writes back. Never auto-installs
# missing extractors.
#
...
# and ../../ato-source-sharepoint/scripts/sharepoint-walk-extract.sh. The
...
usage() {
  cat >&2 <<EOF
Usage: onedrive-walk-extract.sh \\
  --candidates-json <path> \\
  --staging-dir <path> \\
  [--cache-subdir <name>=onedrive-cache] \\
  [--excerpt-subdir <name>=onedrive-excerpts] \\
  [--manifest-name <name>=onedrive-manifest.json] \\
  [--source <name>=onedrive] \\
  [--max-bytes <n>=52428800] \\
  [--scan-id <id>=auto]

Reads the candidates JSON (an array; each entry has site_url,
server_relative_url, filename, size_bytes, mtime, user_upn), downloads
each file via m365, extracts a short excerpt, and emits a manifest the
ato-doc-summarizer agent consumes.
...
CANDIDATES_JSON=""
STAGING_DIR=""
CACHE_SUBDIR="onedrive-cache"
EXCERPT_SUBDIR="onedrive-excerpts"
MANIFEST_NAME="onedrive-manifest.json"
SOURCE="onedrive"
MAX_BYTES=52428800
SCAN_ID=""
...
files_jsonl="$(mktemp -t od-walk-files.XXXXXX)"
skipped_jsonl="$(mktemp -t od-walk-skipped.XXXXXX)"
...
# DUPLICATED in smb-walk-extract.sh and sharepoint-walk-extract.sh — keep in sync.
...
while IFS= read -r line; do
  ...
  user_upn=$(jq -r '.user_upn // ""'             <<<"$line")

  if [ -z "$site_url" ] || [ -z "$server_rel" ] || [ -z "$filename" ] || [ -z "$user_upn" ]; then
  ...
  if [ ! -s "$cache_file" ]; then
    if ! m365 spo file get \
            ...
            --path "$cache_file" \
            >/dev/null 2>&1; then
      # Distinguish forbidden from generic failure when possible.
      # m365 doesn't expose HTTP status cleanly; treat all as download_failed.
      ...
      jq -nc \
        ...
        --arg missing_tool "" \
        --arg user "$user_upn" \
        '{path:$path, reason:$reason, type:$type, missing_tool:$missing_tool, user_upn:$user}' \
        >> "$skipped_jsonl"
      continue
    fi
  fi
  ...
  # uri = full https URL for the OneDrive file
  host_portion="${site_url%%/personal/*}"
  [ "$host_portion" = "$site_url" ] && host_portion="${site_url%/}"
  ...
  jq -nc \
    ...
    '{id:$id, path:$path, uri:$uri, size_bytes:$size, mtime:$mtime,
      ...
      filename_hint:$hint,
      source_meta: {site_url:$site, filename:$filename, user_upn:$user}}' \
    >> "$files_jsonl"
...
{
  echo "onedrive-walk-extract: $candidates candidates, $extracted extracted,"
